...
def create_welcome_main_menu_keyboard(user_keys: list, trial_available: bool, is_admin: bool) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
...
def create_host_selection_keyboard(hosts: list, action: str) -> InlineKeyboardMarkup:
    ...
    builder.button(text="⬅️ Назад", callback_data="manage_keys_photo" if action == 'new' else "back_to_main_menu")
    builder.adjust(1)
    return builder.as_markup()
...
def create_plans_keyboard(plans: list[dict], action: str, host_name: str, key_id: int = 0) -> InlineKeyboardMarkup:
    ...
    back_callback = "manage_keys_photo" if action == "extend" else "buy_new_key"
