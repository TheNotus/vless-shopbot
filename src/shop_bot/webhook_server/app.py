...
from shop_bot.data_manager.database import (
    ...
    get_user, get_key_by_id, delete_key_by_email, get_next_key_number, add_new_key,
    delete_key_by_id, delete_user_and_related_data
)
from shop_bot.data_manager.database import get_user_by_id
...
def create_webhook_app(bot_controller_instance):
    ...
    def login_required(f):
        ...
        def decorated_function(*args, **kwargs):
            logger.info(f"Checking login status - Session contents: {dict(session)}")
            if 'logged_in' not in session:
                logger.info("User not logged in, redirecting to login page")
                return redirect(url_for('login_page'))
            logger.info("User is logged in, allowing access to route")
    ...
    def login_page():
        ...
        logger.info(f"Login attempt - Session contents: {dict(session)}")
        if request.method == 'POST':
            username = request.form.get('username')
            password = request.form.get('password')
            expected_username = settings.get("panel_login")
            expected_password = settings.get("panel_password")
            
            logger.info(f"Login attempt with username: {username}, expected: {expected_username}")
            logger.info(f"Password match: {password == expected_password}")
            
            if username == expected_username and password == expected_password:
                session['logged_in'] = True
                logger.info(f"Login successful - Session contents: {dict(session)}")
    ...
    @login_required
    def logout_page():
        logger.info(f"Logging out - Session contents before: {dict(session)}")
        session.pop('logged_in', None)
        logger.info(f"Logged out - Session contents after: {dict(session)}")
        flash('Вы успешно вышли.', 'success')
        return redirect(url_for('login_page'))

    def get_common_template_data():
        bot_status = _bot_controller.get_status() if _bot_controller else "Not initialized"
        settings = get_all_settings()
    ...
    def delete_user_route(user_id):
        success = delete_user_and_related_data(user_id)
        if success:
            flash(f'Пользователь {user_id} и все его данные были успешно удалены.', 'success')
        else:
            flash(f'Произошла ошибка при удалении пользователя {user_id} и его данных.', 'danger')
        return redirect(url_for('users_page'))
    ...
    def user_profile_page(user_id):
        """
        Маршрут для отображения профиля пользователя и его ключей.
        """
        user = get_user(user_id)
        user_keys = get_user_keys(user_id)
        servers = get_all_hosts()
        ...
        common_data = get_common_template_data()
        return render_template('user_profile.html', user=user, user_keys=user_keys, servers=servers, **common_data)
    @flask_app.route('/delete_key/<int:key_id>', methods=['POST'])
    @login_required
    def delete_key_route(key_id):
        """
        Маршрут для удаления ключа по его ID.
        """
        ...
        try:
            # Удаляем ключ с XUI сервера
            xui_result = asyncio.run(xui_api.delete_client_on_host(key_info['host_name'], key_info['key_email']))
            
            if xui_result:
                # Удаляем ключ из базы данных
                delete_key_by_email(key_info['key_email'])
                flash("Ключ успешно удален", 'success')
            else:
                flash("Не удалось удалить ключ с XUI сервера", 'danger')
        except Exception as e:
            logger.error(f"Error deleting key {key_id}: {e}", exc_info=True)
            flash("Произошла ошибка при удалении ключа", 'danger')
        
        return redirect(url_for('users_page'))

    @flask_app.route('/create_key/<int:user_id>', methods=['POST'])
    @login_required
    def create_key_route(user_id):
        """
        Маршрут для создания нового ключа для пользователя.
        Принимает POST-запрос с информацией о сервере и длительности ключа.
        """
        
        try:
            # Получаем данные из формы
            server_id = request.form.get('server')
            days = int(request.form.get('days', 30))  # по умолчанию 30 дней
            key_name = request.form.get('key_name')  # получаем имя ключа из формы
            ...
            # Генерируем уникальный email для ключа
            key_number = get_next_key_number(user_id)
            key_email = f"user_{user_id}_key_{key_number}"
            
            # Создаем ключ на XUI сервере
            xui_result = asyncio.run(xui_api.create_or_update_key_on_host(server_id, key_email, days))
            
            if xui_result:
                # Сохраняем информацию о ключе в базе данных
                # Используем email пользователя и количество дней из формы
                user_email = user.get('email', f'user_{user_id}@example.com')  # используем email из профиля пользователя или генерируем
                new_key_id = add_new_key(
                    ...
                    days=days,
                    key_email=key_email,
                    expiry_timestamp_ms=xui_result['expiry_timestamp_ms'],
                    key_name=key_name  # передаем имя ключа в функцию добавления
                )
                
                if new_key_id:
                    flash("Ключ успешно создан", 'success')
                else:
                    flash("Не удалось сохранить ключ в базе данных", 'danger')
            else:
                flash("Не удалось создать ключ на XUI сервере", 'danger')
        except Exception as e:
            logger.error(f"Error creating key for user {user_id}: {e}", exc_info=True)
            flash("Произошла ошибка при создании ключа", 'danger')
    ...
    def add_host_route():
        """
        Маршрут для добавления нового хоста.
        """
        create_host(
