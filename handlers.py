...
PHOTO_PATH = BASE_DIR / "media" / "main_menu.jpg"
MENU_PHOTO_FILE_ID: str | None = None

async def _get_photo_file_id(bot) -> str | None:
    global MENU_PHOTO_FILE_ID
    if MENU_PHOTO_FILE_ID:
        return MENU_PHOTO_FILE_ID

    if not PHOTO_PATH.exists():
        print("ФОТО НЕ НАЙДЕНО:", PHOTO_PATH)
        return None
    ...
    try:
        ...
        MENU_PHOTO_FILE_ID = msg.photo[-1].file_id
        print("file_id получен:", MENU_PHOTO_FILE_ID)
        await msg.delete()
        return MENU_PHOTO_FILE_ID
...
async def show_main_menu(
    target: types.Union[types.Message, types.CallbackQuery],
    edit_message: bool = False,
    use_photo: bool = False  # ← правильное имя
):
    # === Определяем message и bot ===
    ...
    trial_available = not (user_db_data and user_db_data.get('trial_used', False))
    ...
    try:
        if use_photo:
            file_id = await _get_photo_file_id(bot)
            if edit_message:
                media = types.InputMediaPhoto(media=file_id, caption=text, parse_mode='HTML')
                await message.edit_media(media=media, reply_markup=keyboard, use_photo = True)
            else:
